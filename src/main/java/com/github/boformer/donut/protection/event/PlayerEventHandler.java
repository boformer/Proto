...
public class PlayerEventHandler 
{
	...
	public PlayerEventHandler(DonutProtectionPlugin plugin) {
		this.plugin = plugin;
	}
	...
	public void onBlockBreak(BlockBreakEvent event) //TODO replace dummy event when supported
	{
		...
		if(world.getProtectionType() == ProtectionType.FREEBUILD) return;
		...
		{
			plot = plugin.getDataStore().getPlot(event.getLocation());
		...
		if(world.getProtectionType() == ProtectionType.WHITELIST) 
		{
			...
			if(plugin.getDataStore().hasPermission(player, world, Permissions.BUILD)) return;
